...
export async function onRequestGet({ env, params }) {
  ...
  const row = await env.DB.prepare(
    "SELECT id, name, description, content, image_url FROM items WHERE id = ?"
  ).bind(id).first();
...
export async function onRequestPut({ request, env, params }) {
  ...
  await env.DB.prepare(
    "UPDATE items SET name = ?, description = ?, content = ?, image_url = ?, updated_at = unixepoch() WHERE id = ?"
  ).bind(name.trim(), description || "", content || "", image_url || "", id).run();
